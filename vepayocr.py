#!/usr/bin/env python3
"""VEPay OCR: extract Venezuelan mobile payment receipts from screenshots.

The script intentionally uses only the Python standard library plus the
Tesseract executable. On Windows it can also call PowerShell/.NET for one
targeted crop that improves BDV amount extraction.
"""
...
SCHEMA_VERSION = "ve_bank_payment_receipt_v1"
APP_NAME = "VEPay OCR"
APP_VERSION = "0.1.0"
...
def targeted_ocr_passes(
    ...
    *,
    lang: str = DEFAULT_LANG,
    enable_windows_crops: bool = True,
) -> list[dict[str, str]]:
    ...
    if not should_try_bdv_amount or not enable_windows_crops:
        return passes
    ...
    try:
        ...
        created = windows_threshold_crop(
            image_path,
            crop_path,
...
def build_receipt(
    ...
    *,
    ...
    extra_passes = targeted_ocr_passes(
        ...
        primary_text,
        lang=lang,
        enable_windows_crops=enable_windows_crops,
    )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "VEPay OCR extrae datos de pago desde capturas de Bancamiga, "
            "Banesco, BDV y Mercantil."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent(
            """
            Ejemplos:
              python vepayocr.py "C:\\pagos\\bancamiga.jpeg"
              python vepayocr.py "C:\\pagos" --format jsonl --output pagos.jsonl
              python vepayocr.py *.jpeg --format csv --output pagos.csv
            """
        ).strip(),
    )
    ...
    parser.add_argument(
        "--no-windows-crops",
        action="store_true",
        help="Desactiva recortes PowerShell/.NET para mejorar campos dificiles como monto BDV.",
    )
    ...
    try:
        ...
        receipts = [
            build_receipt(
                ...
                enable_windows_crops=not args.no_windows_crops,
            )
            for image in images
        ]
